...
`water-flow-chart.md`, `ai-usage-audit.md`, `ledger-and-reconciliation-flow.md`,
`bank-statement-upload-flow.md`, `brand-ledger-flow.md`, and the `agents/` board.
...
| Plan | State |
|---|---|
| `0909-expense-multi-entry-flow-plan.md` | — |
| `0909-po-ai-upload-and-brand-stock-removal-plan.md` | the questions in §4 are unanswered; no code written |
| `0909-po-sheet-ai-extraction-and-catalogue-free-lines-plan.md` | 9 Sep 2026, P1–P4 building on feat/0909-stock-po-expense (owner: same branch); migration po_sheet_extraction_and_line_name applied to local bch |
| `0909-stock-screens-size-category-and-sidebar-plan.md` | nothing built; **no migration at all**, and it depends on nothing. The four independent screen changes split out of the 9 Sep stock conversation: brand-count starts at the store then that store's warehouses, inbound detail shows the time not just the date, the Warehouses sidebar entry goes (by setting `route: null`, **not** deleting the module — its `create/edit/delete` grants gate `/api/warehouses`, which is how anyone adds a floor on `/stores`), and the `/stock/[id]` free-text **Size** input becomes a searchable **Category** picker while the Size filter leaves `/stock`. Settled by measurement against the local `bch` database on 9 Sep: **0 of 5,738 products carry a `size`**, while **20 of 32 categories are wheel sizes holding 3,495 products (~61%)** — the category tree already carries what `size` was for. Dropping the column itself is deferred one release (rule 7). Sibling of `0909-stock-store-and-warehouse-scoping-plan.md`; ships in either order, though both edit neighbouring lines of `stock/page.tsx` |
| `0909-stock-store-and-warehouse-scoping-plan.md` | nothing built. Stock becomes visible at **two scopes**: the shop floor becomes a real location and a store is the sum of its locations. Owner, 9 Sep: *"the store stock is not the sum of ware house … show it as total stock where it shows S+W and the count"*. The shop floor is a second `Warehouse` row per store plus one additive `Warehouse.kind {FLOOR,GODOWN}` column — `StockLevel` stays keyed `(productId, warehouseId)`, so all ~20 existing stock writers, transfers, inbound and audit scoping keep working untouched; a `StoreStock` table and a `storeId` on `StockLevel` were both considered and rejected with reasons. Adds a **By Store** screen beside By Location and a searchable **Store filter** on `/stock` that scopes the quantities, not just the rows. Carries **the only migration** (`warehouse_kind`), and its Part A answers the transfer-mode plan's blocking Q1 (§6). Two traps recorded: Prisma cannot `orderBy` a relation aggregate, so the scoped path pages in memory; and the In Stock / No Stock chips filter `currentStock` and will contradict a scoped column unless changed with it. Split from the combined 9 Sep plan together with `0909-stock-screens-size-category-and-sidebar-plan.md` |
| `0909-transfer-mode-and-document-attachment-plan.md` | — |
| `0909-vendor-ledger-screens-and-ai-import-plan.md` | 9 Sep 2026, every question in §1 answered by the owner the same day; **not started** until the owner says "implement" and names the base branch (Q9). |
