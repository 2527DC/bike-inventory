...
| `1509-po-sheet-mrp-price-and-pdf-columns-plan.md` | **BUILT 15 Sep 2026 on `feat/remove-static-team-health`**, committed and pushed on the owner's instruction; awaiting the owner's `npm run build` and the §4 browser walk. The sheet's MRP (else its BDP/Price) becomes the PO line's unit price, shown read-only and re-read by the server from the review row; GST defaults to 0; a row with no price cannot be selected; the PO PDF drops the SKU and HSN columns. No migration — it reuses `PoExtractionItem.price/mrp`. |
