...
| `1609-deliveries-floor-warehouse-contact-and-self-fill-plan.md` | 21 Sep 2026, all three phases merged in PR #50: the FLOOR warehouse sells, holds and reduces stock; the customer is saved and schedules through the self-fill link; one detail screen with paid / balance and Bangalore / Outstation zones. Build passed 16 Sep; browser walk outstanding. |
| `1709-priority-build-and-stock-flow-plan.md` | 21 Sep 2026, all four phases merged in PR #52: units in sync with stock, the outward floor guard, ★ priority, one approval model with push actions, one-tap hold, the new sidebar and single dashboard, bins and categories, Google Contacts. Parts superseded by 2109 (Generate codes, Unmatched tab, per-rule Apply; inbound off /approvals). |
| `2109-bin-audit-lists-rule-products-plan.md` | 21 Sep 2026: a bin audit lists every active product matching BOTH the brand and the category of the bin's rules; rules now require a brand and a category, and older incomplete ones are flagged on /bins. |
| `2109-inbound-bins-navigation-fixes-plan.md` | 21 Sep 2026, pushed on `feat/2109-inbound-bins-audit-fixes`: a bin on every inbound line with the rule bin locked; inbound off /approvals with quick filters; a bin count changes only its bin (R33) and creates U- codes; bins always on; permission gaps; vendor contact on Vendor. Build passed + production smoke test; owner owes migrate deploy, backfill and seed-rbac elsewhere. |
| `2109-stock-list-table-and-compact-cards-plan.md` | 21 Sep 2026: /stock is a sortable table on PC (≥ 1024 px, sort in the URL) and compact three-line cards with a ⋮ action menu on phone, both rendered from one shared row contract. |
| `2209-audit-assigns-product-bin-plan.md` | 22 Sep 2026: approving a bin audit gives each counted product with no bin that bin; the /stock bin filter matches the home bin OR the items OR a bin quantity; "Needs details" no longer flags binned stock. |
| `2209-permissions-module-search-plan.md` | 22 Sep 2026: a "Search modules…" box on /team/permissions and the gaps matrix, matching the module name; a matching child brings its parent, a matching parent its children; hidden modules keep their grants. |
| `2209-transfers-list-table-and-cards-plan.md` | 22 Sep 2026: /transfers is a table on PC and compact cards on phone, from one shared row contract; tapping anywhere opens the transfer, Approve / Reject act in place. |
...
| `2309-mobile-nav-email-notifications-category-size-plan.md` | rewritten 23 Sep 2026 after the owner answered the questions one by one (§1.1). |
| `2409-stock-audit-push-notifications-plan.md` | built 24 Sep 2026 on `feat/audit-assembly-push`: pushes for stock audit assigned / completed (approvers) / approved / rejected, and one push per assembly assignment. Not yet tested with real pushes. |
| `2409-zoho-vendor-id-on-bill-import-plan.md` | built 24 Sep 2026 on `feat/zoho-vendor-id`: `Vendor.zohoVendorId`, the bill import resolves vendors by Zoho id (no name match) and creates them once with their Zoho details. Migration applied to the test database only. |
| `2409-zoho-vendor-sync-and-issue-restore-plan.md` | **Part A built 24 Sep 2026** on `feat/zoho-vendor-sync` (from `origin/main` @ `045c89d`); |
