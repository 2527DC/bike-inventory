...
| **new** `api/deliveries/[id]/find-stock/route.ts` | **R45, P15** — `deliveries.edit`, not Dummy. `GET`: for each short line, every active warehouse holding usable stock (`quantity − reservedQuantity`), same store first, then other stores; per source: store, warehouse, kind, qty, assembled/unassembled units. `POST {sources:[{productId, warehouseId, qty}]}`: creates **one transfer order per source warehouse** to the outward's floor, `PENDING`, `deliveryId` set, no document (P16); mode from the pair (`GODOWN_TO_FLOOR` / `STORE_TO_STORE` / …); reserves the picked units for the outward (`pickUnits` + `reservedForDeliveryId`); `recordApprovalEvent(REQUESTED)`; notify `approval.requested` to `transfers.approve` holders after commit |
| `deliveries/[id]/_components/stock-hold-card.tsx` (+ detail actions) | when the hold is short: **Find stock** button → sheet listing sources → pick source(s) and qty → **Create transfer request**; the card then shows each linked transfer with its status |
...
| 2 | C | `src/lib/deliveries/*`, `api/deliveries/**` (incl. `find-stock`), `api/public/delivery/**`, `deliveries/**` screens, `src/lib/approvals/actions/delivery.ts` | B |
