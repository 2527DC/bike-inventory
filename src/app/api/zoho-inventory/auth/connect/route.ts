...
import { exchangeGrantTokenInventory } from "@/lib/zoho-inventory";
import { successResponse, errorResponse } from "@/lib/api-utils";
import { requireFeature, AuthError } from "@/lib/auth-helpers";

// POST: Connect to Zoho Inventory using grant token (self-client flow)
export async function POST(req: NextRequest) {
  try {
    ...
    const tokens = await exchangeGrantTokenInventory(clientId, clientSecret, grantToken);

    const expiresAt = new Date(Date.now() + tokens.expiresIn * 1000);

    // Upsert the singleton config
    await prisma.zohoInventoryConfig.upsert({
      where: { id: "singleton" },
      update: {
        clientId,
        clientSecret,
    ...
    return errorResponse(error instanceof Error ? error.message : "Failed to connect to Zoho Inventory", 500);
  }
}
