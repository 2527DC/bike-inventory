...
import { Search, QrCode, Camera, X, Package, AlertCircle, CheckCircle2 } from "lucide-react";
...
const log = createLogger("scanner:panel");
...
/**
 * Barcode scan + search, without a page header. Rendered by `/scanner` and by the Labels tab on
 * `/assembly?tab=labels` (plan 1709, R29), so the two can never drift apart.
 */
export function ScannerPanel() {
  ...
  const handleSearch = useCallback(async (code: string) => {
    ...
    setSearched(true);
    setSearchError("");
    const { data, error, status } = await apiTry<{ serials?: SerialResult[]; products?: ProductResult[] }>(
      `/api/serials/search?code=${encodeURIComponent(code)}`
    );
    setLoading(false);
    if (error || !data) {
      log.error("search failed", { status, length: code.length });
  ...
  const stopCamera = useCallback(async () => {
    if (html5ScannerRef.current) {
      try {
        ...
        log.debug("camera stop ignored", { message: err instanceof Error ? err.message : String(err) });
      }
  ...
  // Cleanup on unmount — a tab switch unmounts this too, and the camera must not stay on.
  useEffect(() => {
    return () => {
      if (html5ScannerRef.current) {
        html5ScannerRef.current.stop().catch((err: unknown) => {
          log.debug("camera stop on unmount ignored", { message: err instanceof Error ? err.message : String(err) });
        });
      }
    };
  }, []);
  ...
  const startCamera = async () => {
    try {
      ...
      setCameraActive(false);
      log.warn("camera start failed", { message: err instanceof Error ? err.message : String(err) });
                ...
                <Link key={p.id} href={`/stock/${p.id}`}>
                  <Card className="hover:bg-slate-50 transition-colors">
                    <CardContent className="p-3">
                      ...
                      <div className="flex flex-wrap gap-3 text-xs mt-1.5 tabular-nums">
              ...
              {serials.map((s) => (
                <Card key={s.id}>
                  <CardContent className="p-3">
                    ...
                    <div className="flex flex-wrap gap-3 text-xs mt-2 tabular-nums">
